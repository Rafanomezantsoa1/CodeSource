...
set TEST_DIR=projetTest
set TOMCAT_HOME=C:\apache-tomcat-10.1.34
set TOMCAT_DIR=%TOMCAT_HOME%\webapps
set FRAMEWORK_JAR=framework-1.0.0.jar
set TEST_WAR=projetFrameworkLalaina.war
set APP_NAME=projetFrameworkLalaina
...
:: Suppression de l'ancien WAR et dossier deploye
...
if exist "%TOMCAT_HOME%\work\Catalina\localhost\%APP_NAME%" (
    rmdir /s /q "%TOMCAT_HOME%\work\Catalina\localhost\%APP_NAME%" >nul 2>&1
    echo ✓ Dossier work de Tomcat supprime
)
...
if not exist "%TEST_DIR%\target\projetTestFramework-1.0.0.war" (
    echo ERREUR: Le fichier WAR n'existe pas: %TEST_DIR%\target\projetTestFramework-1.0.0.war
    pause
    exit /b 1
)

:: Copie du nouveau WAR avec le nom fixe
echo Copie de %TEST_DIR%\target\projetTestFramework-1.0.0.war vers %TOMCAT_DIR%\%TEST_WAR%
copy "%TEST_DIR%\target\projetTestFramework-1.0.0.war" "%TOMCAT_DIR%\%TEST_WAR%" >nul
if %ERRORLEVEL% neq 0 (
    echo ERREUR: Echec de la copie du WAR vers Tomcat
    echo Source: %TEST_DIR%\target\projetTestFramework-1.0.0.war
