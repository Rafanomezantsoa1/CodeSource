...
public class ScannerPackage {
    ...
    Map<String, Method> dynamic = new HashMap<>();
    ...
    for (Class<?> clazz : controllers) {
        ...
        for (Method method : clazz.getDeclaredMethods()) {
            if (method.isAnnotationPresent(AnnotationUrl.class)) {
                ...
                String full = baseUrl + url.url();

                if (full.contains("{")) {
                    // Transformer /dept/{id} en regex : ^/dept/[^/]+$
                    String regex = full.replaceAll("\\{[^/]+\\}", "[^/]+");
                    dynamic.put("^" + regex + "$", method);
                } else {
                    exact.put(full, method);
                }
            }
        }
    }

    // Fusionner exact + dynamic dans un seul map
    Map<String, Method> finalMap = new HashMap<>();
    finalMap.putAll(exact);
    finalMap.putAll(dynamic);
    return finalMap;
}
